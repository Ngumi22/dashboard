...
export async function addBrand(formData: FormData) {
  ...
  try {
    ...
    const created_by = formData.get("created_by") as string; // This should be from the authenticated user
    ...
    // Convert brand_logo to binary data
    ...
    const [result]: [ResultSetHeader, FieldPacket[]] = await connection.query(
      `
        INSERT INTO brands (name, brand_logo, created_by)
        VALUES (?, ?, ?)
      `,
...
export async function createTag(formData: FormData) {
  ...
  try {
    ...
    const [existingTag]: [RowDataPacket[], FieldPacket[]] =
      await connection.query("SELECT id FROM tags WHERE name = ?", [tagName]);
...
export async function createProductTags(
  ...
  try {
    ...
    for (const tag of tags) {
      ...
      const [existingTag]: [RowDataPacket[], FieldPacket[]] =
        await connection.query("SELECT tag_id FROM tags WHERE name = ?", [
          tagName,
        ]);
...
export async function uploadVariantImages(
  ...
  try {
    ...
    if (!allImagesValid) {
      throw new CustomError(
        "Images must be less than 100KB and in JPEG or PNG format.",
        400
      );
    }
    ...
    const query = `
      INSERT INTO product_variant_images (variant_id, main_image, thumbnail_image1, thumbnail_image2, thumbnail_image3, thumbnail_image4, thumbnail_image5)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `;

    const [result]: [ResultSetHeader, FieldPacket[]] = await connection.query(
      query,
      [variantId, mainImageBuffer, ...thumbnailBuffers]
    );
